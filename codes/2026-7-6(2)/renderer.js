...
(async () => {
  ...
  const FEAT_BODY = {
    ...
    cloud: 'body-cloud', offline: 'body-offline',
  };
  function applyFeature(key) {
    switch(key) {
      ...
      case 'record':
        ...
        break;
      case 'cloud':
        btnSaveCloud.style.display = features.cloud ? '' : 'none';
        if (!features.cloud) { cloudStatus.textContent = '대기 중'; cloudUrl.style.display = 'none'; }
        break;
      case 'offline':
        updateOfflineBadge();
        break;
  ...
  function updateOfflineBadge(){
    ...
    if(features.offline){
      ...
      if(features.cloud){ cloudStatus.textContent='오프라인 모드 — 클라우드 저장 불가'; cloudUrl.style.display='none'; }
    } else {
      badge.style.display='none';
      if(features.cloud){ cloudStatus.textContent='대기 중'; }
    }
  }
  async function saveToCloud(){
    ...
    cloudStatus.textContent='저장 중…';cloudUrl.style.display='none';
    ...
    try{
      ...
      cloudStatus.textContent=`저장 완료 → #${newId}`;
      cloudUrl.textContent=url;cloudUrl.style.display='block';
      cloudUrl.onclick=()=>require('electron').shell.openExternal(url);
      toast(`클라우드 저장 완료! #${newId}\n${url}`,'ok',6000);
    }catch(err){
      console.error('[saveToCloud]',err);
      cloudStatus.textContent='저장 실패: '+err.message;
