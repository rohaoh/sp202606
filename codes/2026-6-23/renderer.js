(async () => {
  ...
  function replayRecording(r){
    ...
    playing=true; playHead=0; lastHighlightedRow=null; requestRender();
  }
  ...
  btnReset.addEventListener('click',()=>{
    playing=false;playHead=0;impacted=false;bouncing=false;
  ...
  function animLoop(now){
    ...
    if(playing&&simResult){
      ...
      if(features.multiobj){
        moObjects.forEach((o,i)=>{
          ...
          const fi2=o.result.frames.findIndex(f=>f.t>=playHead);
          const fr2=fi2>=0?o.result.frames[fi2]:o.result.frames[o.result.frames.length-1];
      ...
      if(camFollowMode){
        // 부드럽게 공을 따라가도록 orbitTarget을 lerp + 가까운 거리 유지
        orbitTarget.lerp(fallingMesh.position,0.18);
        const targetR=Math.max(12,fallingMesh.position.y*0.08+12);
        orbitRadius+=(targetR-orbitRadius)*0.1;
      } else {
    ...
    if(bouncing&&fallingMesh){
      ...
      if(camFollowMode){orbitTarget.lerp(fallingMesh.position,0.18);updateCamera();}
      needsRender=true;
    }
