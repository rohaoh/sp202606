(async () => {
  ...
  const LEVEL_CLASS = {
    'No Damage':'lv0','Minor Damage':'lv1','Moderate Damage':'lv2',
  ...
  function rebuildFallingMesh() {
    ...
    fallingMesh=new THREE.Mesh(buildShapeGeo(selShape.value),FALL_MATS[selShape.value]);
  ...
  function animLoop(now){
    ...
    if(playing&&simResult){
      ...
      if(camFollowMode){orbitTarget.copy(fallingMesh.position);orbitRadius=Math.max(10,currentH0*0.015);}
      else{orbitTarget.y=fallingMesh.position.y*0.5;}
      updateCamera();
