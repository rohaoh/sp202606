...
function render(d) {
  ...
  // 클라우드 저장 상태/링크 — 메인 렌더러가 saveToCloud() 진행/완료 시마다 다시 push해준다.
  const cloudStatusEl = document.getElementById('cloud-status');
  const cloudUrlEl = document.getElementById('cloud-url');
  const btnCloud = document.getElementById('btn-cloud');
  cloudStatusEl.textContent = d.cloudStatus || '대기 중';
  btnCloud.disabled = d.cloudStatus === '저장 중…';
  if (d.cloudUrl) {
    cloudUrlEl.textContent = d.cloudUrl;
    cloudUrlEl.style.display = 'block';
    cloudUrlEl.onclick = () => window.resultsBridge.openExternal(d.cloudUrl);
  } else {
    cloudUrlEl.style.display = 'none';
  }
}
...
document.getElementById('btn-cloud').addEventListener('click', () => window.resultsBridge.action({ action: 'cloud' }));
