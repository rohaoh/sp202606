(async () => {
  ...
  window.addEventListener('mousemove',e=>{
    ...
    lastMouse={x:e.clientX,y:e.clientY};
    if(isDragging){orbitTheta-=dx*0.008;orbitPhi=Math.max(0.05,Math.min(Math.PI*0.48,orbitPhi+dy*0.008));updateCamera();}
    if(isPanning){
      const r=new THREE.Vector3();
      r.crossVectors(camera.getWorldDirection(new THREE.Vector3()),new THREE.Vector3(0,1,0)).normalize();
      orbitTarget.addScaledVector(r,-dx*0.08); orbitTarget.y+=dy*0.08; updateCamera();
    }
  });
  renderer3.domElement.addEventListener('wheel',e=>{
    orbitRadius=Math.max(5,Math.min(2000,orbitRadius+e.deltaY*0.1)); updateCamera();
  });
  ...
  function clearFragments(){
    fragmentMeshes.forEach(m=>scene.remove(m)); fragmentMeshes=[];
    if(dustParticles){scene.remove(dustParticles);dustParticles=null;}
    fracturing=false; fragPending=false;
  }
  ...
  function highlightTable(ph) {
    if (!simResult || !tblBody.children.length) return;
    const key = (Math.floor(ph * 10) / 10).toFixed(1);
    const rows = tblBody.querySelectorAll('tr');
    for (const row of rows) {
      if (row.dataset.t === key) {
        if (lastHighlightedRow && lastHighlightedRow !== row) lastHighlightedRow.classList.remove('highlight');
  ...
  function scheduleGraphRedraw(){
    ...
    graphTimer=setTimeout(()=>drawGraph(activeTab),100);
  }
  ...
  btnRun.addEventListener('click',async()=>{
    ...
    if(window.physics&&tgt){
      const res=await window.physics.simulate({falling,target:tgt,height:+inpHeight.value,gravity:+inpG.value});
      if(res.ok){
        ...
        if(res.data.trajectory){
          res.data.trajectory.forEach((f,i)=>{
            if(i<result.frames.length){result.frames[i].rho=f.airDensity;result.frames[i].atm=f.atmosphere;}
          });
        }
      }
    }
    ...
    const visualH=Math.min(h0,1500);
  ...
  btnPlay.addEventListener('click',()=>{
    if(simResult){playing=true;playStart=performance.now()-playHead/(simResult.fallTime||1)*simResult.fallTime*1000;}
  });
  ...
  function animLoop(now){
    ...
    if(fracturing&&fragmentMeshes.length>0&&window.physics&&!fragPending){
      fragPending=true;
      window.physics.stepFragments(dt,+inpG.value).then(states=>{
        states.forEach((s,i)=>{
    ...
    if(dustParticles){
      ...
      if(dustParticles.material.opacity<=0){scene.remove(dustParticles);dustParticles=null;}
    }
  ...
  window.addEventListener('resize',()=>{if(simResult)scheduleGraphRedraw();});
})();
