...
double PhysicsEngine::calcAirDensity(double alt, double tempOffset, double humidity) {
    ...
    // dP/dh = -rho*g 를 풀면 P, rho 모두 exp(-h/H) 꼴이 된다. H = R*T/g 는 그 층의
    // "스케일 하이트"). 온도는 층마다 다르게 변하므로(대류권 선형 감소, 성층권 등온/역전),
    // 층 경계에서 이전 층 값을 이어받아 그 층 고유의 스케일 하이트로 연속적으로
    // 이어 붙인다 — 그래야 경계에서 값이 툭 끊기지 않고 매끄럽게 지수 감소를 유지한다.
    double T = 0.0, P = 0.0;
    if (alt <= 11000.0) {
        T = T0 - L0 * alt;                  // 대류권: 고도에 따라 온도는 선형으로 감소
        double H0 = Rd * T0 / g;            // 해수면 스케일 하이트
        P = P0 * std::exp(-alt / H0);       // 압력/밀도는 지수적으로 감소
    } else if (alt <= 20000.0) {
        double T11 = T0 - L0 * 11000.0;
        double H0  = Rd * T0 / g;
        double P11 = P0 * std::exp(-11000.0 / H0);
        T = T11;
...
ImpactResult PhysicsEngine::simulate(const SimInput& input) {
    ...
    // [F-ADT] 적응형 타임스텝: 고도 하강 시 공기 밀도 급증 → 드래그 강직성 증가 →
    // 고정 dt로는 γ·dt ≥ 1 조건에서 속도가 단말속도 주변을 진동(과수렴)할 수 있다.
    // 안정 조건: dt < 0.5 / γ, where γ = rho·Cd·A·|v| / (2·m)
    // 고도 정밀도 조건: dt < 200 / |vy| (한 스텝에 200m 이상 변하지 않도록)
    const double DT_MIN  = 0.001; // 최소 타임스텝 (1 ms)
    const double DT_MAX  = 0.1;   // 최대 타임스텝 (100 ms)
    const double DT_FIXED = 0.05; // 비적응 모드 고정 타임스텝
    ...
    double minDt = DT_MAX, maxDt = DT_MIN;
    int totalSteps = 0;
    double lastFrameT = -1.0; // 마지막으로 프레임을 저장한 시각
    ...
    while (altitude > 0.0 && t < 7200.0) {
        ...
        // [F-ADT] 적응형 타임스텝 계산
        // γ = dragBase / mass (유효 드래그 감쇠율, 단위: 1/s)
        // 과수렴 조건: γ·dt ≥ 1 → 속도가 단말속도를 중심으로 진동한다.
        // 안정성 기준: dt ≤ 0.5 / γ (safety factor 0.5 적용)
        // 고도 정밀도 기준: dt ≤ 200 / |vy| (한 스텝에 200m 이상 변하지 않도록)
        double dt;
        if (input.adaptiveDt) {
            double gamma = (obj.mass > 0.0 && vRelMag > 0.0)
                           ? dragBase / obj.mass : 0.0;
            double dt_stab = (gamma > 1e-10) ? (0.5 / gamma) : DT_MAX;
            // 한 스텝에 고도의 50% 또는 200m 이상 변하지 않도록 제한
            double altLimit = std::min(altitude * 0.5, 200.0);
            double dt_alt  = (std::abs(vy) > 0.1) ? (altLimit / std::abs(vy)) : DT_MAX;
            dt = std::min({DT_MAX, dt_stab, dt_alt});
        ...
        // 신규: 물 저항 추가
        // 수면은 지면(고도 0) 기준 input.waterDepth 만큼 위에 있다. 따라서 물체가
        // 수면 아래로 내려간 깊이는 (waterDepth - altitude)이다. 이전 코드는
        // (input.height - altitude)를 사용해 낙하 시작 직후(고고도)부터 이미
        // 물속에 있는 것으로 잘못 계산했다.
        double waterDepth = std::max(0.0, input.waterDepth - altitude);
        ...
        // 적응형 모드에서는 0.05s 간격 또는 충돌 직전 프레임을 항상 기록한다
        // (매 내부 스텝마다 저장하면 메모리가 폭증한다).
        // 고정 dt 모드에서는 기존처럼 매 스텝 저장한다.
        bool willHitGround = (altitude + vy * dt) <= 0.0;
        bool shouldRecord = !input.adaptiveDt || (t - lastFrameT >= 0.05 - 1e-9) || willHitGround;
        if (shouldRecord) {
            PhysicsFrame frame;
            frame.time         = t;
            frame.velocity     = vy;
        ...
        vy       += accel_y * dt;
        // vy는 y-down 부호 규약(음수 = 하강)을 쓰므로, 고도는 vy를 더해서
        // 낮춰야 한다. 기존의 "altitude -= vy*dt"는 부호가 뒤집혀 있어 물체가
        // 하강 가속을 받는데도 고도가 계속 올라가는 버그를 일으켰고,
        // 그 결과 물 저항 트리거 오류(waterDepth 계산)와 겹쳐 충돌 직전
        // 속도가 갑자기 폭증하는 등 낙하 시뮬레이션 전반이 부정확했다.
        altitude += vy * dt;
        vx       += accel_x * dt;
        vz       += accel_z * dt;
        posX     += vx * dt;
    ...
    result.minDtUsed  = (totalSteps > 0) ? minDt : DT_FIXED;
    result.maxDtUsed  = (totalSteps > 0) ? maxDt : DT_FIXED;
    ...
    const std::vector<PhysicsFrame>& traj)
{
    ...
    // impactVelocity arrives signed (negative = downward, per the simulate() loop's
    // y-down convention). Impact momentum/force/energy are reported as magnitudes,
    // matching terminalVelocity — so normalize the sign once, here, instead of
    // scattering std::abs() calls (and missing some) at every call site.
    impactVelocity = std::abs(impactVelocity);
    ...
    // Heavier/larger impactors compress and deform over more material during contact,
    // which stretches out the collision pulse (this is why a bowling ball has a longer,
    // gentler crash pulse than a golf ball at the same speed). Without this, the average
    // deceleration during impact (~impactVelocity/collisionTime) was completely
    // independent of the falling object's mass — only the target's stiffness and the
    // impact speed mattered. Reference mass ~10 kg (roughly bowling-ball scale); the
    // exponent is kept small (0.12) so this stays a secondary correction — total impact
    // force still grows with mass overall (via momentum J), this just keeps heavier
    // objects from experiencing the exact same instantaneous deceleration as light ones.
    double massFactor = std::pow(std::max(obj.mass, 0.01) / 10.0, 0.12);
    baseTime *= massFactor;

    double velFactor = std::max(0.15, std::min(1.0, 20.0 / (std::abs(impactVelocity) + 1e-6)));
    double collisionTime = std::clamp(baseTime * velFactor, 0.0002, 0.05);
    ...
    // Brittle materials fail at much lower average pressure than their static
    // strength rating because dynamic/impact loading concentrates stress at the
    // contact point instead of spreading it, and the material has no time to
    // yield plastically before failing. Ductile materials (deform mode, e.g.
    // steel) don't get this reduction — they dent/bend instead of cracking.
    //   - shatter (glass): lowest toughness, fails almost immediately past yield
    //   - wood: fibrous, splits easily along the grain under a sudden point
    //     impact even though its static bending strength looks high on paper
    //   - concrete/brick: brittle masonry, moderate reduction
    double brittleFactor = 1.0;
    if (target.fractureMode == "shatter")       brittleFactor = 8.0;
    else if (target.material == "wood")         brittleFactor = 3.0;
    else if (target.material == "concrete" ||
             target.material == "brick")        brittleFactor = 1.4;
    ...
    std::vector<Fragment>& fragments, double dt, double gravity)
{
    ...
    for (auto& frag : fragments) {
        ...
        if (frag.position[1] < -0.5f) {
            ...
            // 지면 접촉 시 세 축 모두 감쇠시킨다. 이전에는 X/Z축만 감쇠하고
            // 수직축(Y, 요잉 회전)은 그대로 두어 바닥에 닿아 멈춘 뒤에도
            // 계속 팽이처럼 도는 것처럼 보이는 버그가 있었다.
            frag.angularVelocity[0] *= 0.5f;
            frag.angularVelocity[1] *= 0.5f;
            frag.angularVelocity[2] *= 0.5f;
            if (std::abs(frag.velocity[1]) < 0.1f) {
                frag.active = false;
                frag.angularVelocity = {0.0f, 0.0f, 0.0f};
            }
        }
