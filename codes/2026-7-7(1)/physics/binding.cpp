...
Napi::Value Simulate(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        simInput.windX        = inp.Has("windX")        ? inp.Get("windX").As<Napi::Number>().DoubleValue()        : 0.0;
        simInput.windZ        = inp.Has("windZ")        ? inp.Get("windZ").As<Napi::Number>().DoubleValue()        : 0.0;
        simInput.tempOffset   = inp.Has("tempOffset")   ? inp.Get("tempOffset").As<Napi::Number>().DoubleValue()   : 0.0;
        simInput.humidity     = inp.Has("humidity")     ? inp.Get("humidity").As<Napi::Number>().DoubleValue()     : 50.0;
        simInput.spinRate     = inp.Has("spinRate")     ? inp.Get("spinRate").As<Napi::Number>().DoubleValue()     : 0.0;
        simInput.spinAxisX    = inp.Has("spinAxisX")    ? inp.Get("spinAxisX").As<Napi::Number>().DoubleValue()    : 0.0;
        simInput.spinAxisY    = inp.Has("spinAxisY")    ? inp.Get("spinAxisY").As<Napi::Number>().DoubleValue()    : 0.0;
        simInput.spinAxisZ    = inp.Has("spinAxisZ")    ? inp.Get("spinAxisZ").As<Napi::Number>().DoubleValue()    : 1.0;
        simInput.launchAngle  = inp.Has("launchAngle")  ? inp.Get("launchAngle").As<Napi::Number>().DoubleValue()  : 90.0;
        simInput.launchAzimuth= inp.Has("launchAzimuth")? inp.Get("launchAzimuth").As<Napi::Number>().DoubleValue(): 0.0;
        simInput.terrainSlope = inp.Has("terrainSlope") ? inp.Get("terrainSlope").As<Napi::Number>().DoubleValue() : 0.0;
        simInput.waterDepth   = inp.Has("waterDepth")   ? inp.Get("waterDepth").As<Napi::Number>().DoubleValue()   : 0.0;
        simInput.bounceDamping= inp.Has("bounceDamping")? inp.Get("bounceDamping").As<Napi::Number>().DoubleValue(): 0.5;
        simInput.latitude     = inp.Has("latitude")     ? inp.Get("latitude").As<Napi::Number>().DoubleValue()     : 37.5;
        simInput.adaptiveDt   = inp.Has("adaptiveDt")   ? inp.Get("adaptiveDt").As<Napi::Boolean>().Value()        : false;
        ...
        result.Set("impactEnergy",     Napi::Number::New(env, res.impactEnergy));
        ...
        result.Set("totalEnergyLoss",    Napi::Number::New(env, res.totalEnergyLoss));
        result.Set("coriolisDeflection", Napi::Number::New(env, res.coriolisDeflection));
        result.Set("minDtUsed",          Napi::Number::New(env, res.minDtUsed));
        result.Set("maxDtUsed",          Napi::Number::New(env, res.maxDtUsed));
        result.Set("totalSteps",         Napi::Number::New(env, res.totalSteps));
        ...
        for (size_t i = 0; i < traj.size(); i += step) {
            ...
            frame.Set("posX",        Napi::Number::New(env, traj[i].posX));
            frame.Set("posZ",        Napi::Number::New(env, traj[i].posZ));
            trajArr[idx++] = frame;
        }
...
Napi::Value ComputeFracture(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        impact.impactEnergy     = impObj.Has("impactEnergy") ? impObj.Get("impactEnergy").As<Napi::Number>().DoubleValue() : 0.0;
