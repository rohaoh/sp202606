...
struct SimInput {
    ...
    // [F-ADT] 적응형 타임스텝
    bool   adaptiveDt;   // true: 드래그 강직성에 따라 dt를 자동 조정
};
...
struct PhysicsFrame {
    ...
    double dtUsed;        // [F-ADT] 이 프레임에 사용된 타임스텝 (s)
};
...
struct ImpactResult {
    ...
    // [F-ADT] 적응형 타임스텝 통계
    double minDtUsed;    // 시뮬레이션 중 사용된 최소 dt (s)
    double maxDtUsed;    // 시뮬레이션 중 사용된 최대 dt (s)
    int    totalSteps;   // 실제 계산 스텝 수
};
