// @ts-nocheck — 대규모 레거시 UI 파일이라 점진적 타입 도입 대상에서 제외.
// 신규 파일(physics-core.js, sim-worker.js, main.js 등)은 타입 체크 대상.
...
// 로그인 화면 없이 조용히 익명 인증만 해둔다 — Firestore 보안 규칙이
// request.auth != null 을 요구할 수 있게 하기 위함("missing or insufficient
// permissions" 오류 방지). saveToCloud()에서 쓰기 전에 이 Promise를 기다린다.
const _authReady = signInAnonymously(_auth).catch((err) => {
...
(async () => {
  ...
  const moStaggerRow     = $('mo-stagger-row');
  const inpStaggerStep   = $('inp-stagger-step');
  const btnAutoStagger   = $('btn-auto-stagger');
  ...
  // 결과창(별도 팝업)에도 같은 클라우드 상태/링크를 보여주기 위한 공유 상태.
  // buildResultData()가 이 값을 읽어 results:data 로 결과창에 push 한다.
  let lastCloudInfo = { status: '대기 중', url: null };
  ...
  const selLs            = $('sel-ls');
  const btnLsExport      = $('btn-ls-export');
  const btnLsImport      = $('btn-ls-import');
  const inpLsImportFile  = $('inp-ls-import-file');
  const uVt              = $('u-vt');
  const uVi              = $('u-vi');
  const uOvH             = $('u-ov-h');
  const uOvV             = $('u-ov-v');
  ...
  const FEAT_BODY = {
    ...
    offline: 'body-offline', epsDelta: 'body-epsDelta',
  };
  function applyFeature(key) {
    switch(key) {
      ...
      case 'adaptiveDt':
        updateAdaptiveDtDisplay();
        break;
      case 'epsDelta':
        if (features.epsDelta && simResult) runEpsDeltaAnalysis();
        else clearEpsDeltaDisplay();
        break;
    }
  }
  ...
  (function addAppControls(){
    ...
    sec.innerHTML =
      ...
      '<div class="field"><label>Thermal failure action</label>'+
        '<select id="sel-heat-fail"><option value="off">None</option>'+
        '<option value="burnup">Burn up</option>'+
        '<option value="disintegrate">Disintegrate</option></select></div>'+
      '<div class="field"><label>Surface temp. limit <span>°C</span></label>'+
        '<input type="number" id="inp-heat-threshold" value="1500" min="100" step="50"></div>'+
      '<div class="field"><label>FPS cap</label>'+
        '<select id="sel-fps-cap"><option value="0">Unlimited</option><option value="30">30</option>'+
    ...
    if (alt <= 11000) {
      ...
      if (alt <= 20000) {
        ...
        if (alt <= 32000) {
          ...
          const H20 = R*T11/grav; // T20 == T11
          P=P20*Math.exp(-(alt-20000)/H20);
        } else if (alt <= 47000) {
          return airDensityJS(32000,tempOffset,0)*Math.exp(-(alt-32000)/6500);
        } else if (alt <= 80000) {
          return airDensityJS(47000,tempOffset,0)*Math.exp(-(alt-47000)/7000);
        } else { return 1e-5; }
      }
    }
    let rho = P/(R*T);
  ...
  // [F-ADT] 적응형 타임스텝 통계 표시
  function updateAdaptiveDtDisplay(result) {
    const hint = $('adt-hint');
    if (!hint) return;
    if (!features.adaptiveDt) {
      hint.textContent = `고정 Δt = ${(0.05*1000).toFixed(0)} ms`;
      return;
    }
    if (result && result.minDt != null) {
      const minMs = (result.minDt * 1000).toFixed(2);
      const maxMs = (result.maxDt * 1000).toFixed(2);
      const steps = result.totalSteps != null ? result.totalSteps.toLocaleString() : '—';
      hint.textContent = `Δt: ${minMs}–${maxMs} ms  ·  ${steps} 스텝`;
    } else {
      hint.textContent = '시뮬레이션 후 통계 표시';
    }
  }
  updateAdaptiveDtDisplay();

  // ── [ε-δ] ε-δ Analysis ──
  // ε-δ 논법에 의한 종단속도 수렴 분석:
  //   lim(H→∞) v_impact(H) = v_t
  //   ∀ε>0 ∃H_min: H≥H_min → |v_t − v_impact| < ε·v_t
  //
  // 해석적 해 (1D, 일정 밀도, 수직 낙하):
  //   v(t) = v_t · tanh(g·t/v_t)
  //   h(t) = (v_t²/g) · ln(cosh(g·t/v_t))
  //   v = (1−ε)·v_t ← tanh(τ) = 1−ε ← τ = atanh(1−ε)
  //   H_min = (v_t²/g) · ln(cosh(τ))
  //
  // y_max (과수렴 최대 충격량 높이):
  //   고정 Δt에서 선형화된 드래그 감쇠율 γ = g/v_t
  //   γ·Δt > 1 이면 오일러 적분이 v_t를 중심으로 진동(과수렴).
  //   높이에 따라 충돌 직전 위상이 달라 충격량에 극값이 생긴다.
  //   H_ε(해석해)부터 스캔해 수치적으로 y_max를 찾는다.
  ...
  // 현재 입력 파라미터로 해수면 종단속도 계산
  function calcVtSea() {
    const m = +inpMass.value, A = +inpArea.value, Cd = +inpCd.value, g = +inpG.value;
    if (!m || !A || !Cd || !g) return null;
    const rho = airDensityJS(0, +inpTemp.value || 0, +inpHumidity.value || 50);
    return Math.sqrt((2 * m * g) / (rho * Cd * A));
  }

  // ε → H_min 해석적 계산 (1D, 일정 대기밀도, 수직 낙하)
  function calcHmin(eps) {
    const vt = calcVtSea();
    if (!vt || vt <= 0) return null;
    const g = +inpG.value || 9.81;
    const tau = epsAtanh(1 - eps);
    if (!isFinite(tau)) return null;
    const coshTau = Math.cosh(tau);
    return { hMin: (vt * vt / g) * Math.log(coshTau), vt, g, tau };
  }

  // 고정 Δt에서 주어진 높이의 충격 속도를 빠르게 추정 (경량 수치 시뮬)
  // v*|v| 부호 처리로 상향/하향 모두 올바른 항력 적용
  function simImpactVelocityFixedDt(mass, area, cd, g, h0, tempOff, hum, dt) {
    const rho = airDensityJS(0, tempOff, hum);
    const vt = Math.sqrt((2 * mass * g) / (rho * cd * area));
    let v = 0, h = h0;
    for (let i = 0; i < 500000 && h > 0; i++) {
      const drag = 0.5 * rho * cd * area * v * Math.abs(v);
      const a = g - drag / mass;
      v += a * dt;
      h -= v * dt;
      // 발산 감지: v가 종단속도의 100배를 초과하면 수치 불안정
      if (!isFinite(v) || Math.abs(v) > vt * 100) return NaN;
    }
    return Math.abs(v);
  }

  // y_max 탐색: H_min~N·H_min 범위를 스캔해 최대 충격량 높이를 찾는다.
  // 고정 Δt 시뮬레이션에서 v_impact(H)의 최고점을 탐색.
  function findYmax(hMin, mass, area, cd, g, tempOff, hum) {
    const DT_FIXED = 0.05;  // physics-core.js PC_DT_SIM
    const rho = airDensityJS(0, tempOff, hum);
    const vt = Math.sqrt((2 * mass * g) / (rho * cd * area));
    // 2차 항력을 v_t 근방에서 선형화: da/dv = -2g/v_t (= -ρCdA·v_t/m)
    // 오일러 적분 안정 조건: γ_eff·Δt < 1, where γ_eff = 2g/v_t
    const gamma = 2 * g / vt;
    // γ_eff·Δt ≤ 1 이면 과수렴 없음 → y_max 의미 없음
    if (gamma * DT_FIXED <= 1.0) return { ymax: null, reason: '과수렴 없음 (γ_eff·Δt ≤ 1)' };

    // 스캔 구간: hMin ~ 60·hMin, 200개 샘플
    const N = 200;
    const hStart = Math.max(hMin * 0.5, 1e-4);
    const hEnd   = hMin * 60;
    let bestH = hStart, bestV = 0;
    for (let i = 0; i <= N; i++) {
      const h = hStart + (hEnd - hStart) * (i / N);
      const vi = simImpactVelocityFixedDt(mass, area, cd, g, h, tempOff, hum, DT_FIXED);
      if (isFinite(vi) && vi > bestV) { bestV = vi; bestH = h; }
    }
    if (bestV <= 0) return { ymax: null, reason: '수치 발산 — γ·Δt가 너무 커 탐색 불가' };
    // 황금 탐색으로 정밀화
    let lo = Math.max(hStart, bestH - hMin * 3), hi = Math.min(hEnd, bestH + hMin * 3);
    const phi = (Math.sqrt(5) - 1) / 2;
    for (let iter = 0; iter < 40; iter++) {
      const m1 = hi - phi * (hi - lo), m2 = lo + phi * (hi - lo);
      const v1 = simImpactVelocityFixedDt(mass, area, cd, g, m1, tempOff, hum, DT_FIXED);
      const v2 = simImpactVelocityFixedDt(mass, area, cd, g, m2, tempOff, hum, DT_FIXED);
      const vv1 = isFinite(v1) ? v1 : 0, vv2 = isFinite(v2) ? v2 : 0;
      if (vv1 < vv2) lo = m1; else hi = m2;
    }
    const ymax = (lo + hi) / 2;
    const vmaxImpact = simImpactVelocityFixedDt(mass, area, cd, g, ymax, tempOff, hum, DT_FIXED);
    if (!isFinite(vmaxImpact)) return { ymax: null, reason: '수치 발산 — γ·Δt가 너무 커 탐색 불가' };
    return { ymax, vmaxImpact, vt, gammaDt: gamma * DT_FIXED, reason: null };
  }

  function clearEpsDeltaDisplay() {
    const hminEl = $('eps-hmin'), ymaxEl = $('eps-ymax'), detEl = $('eps-detail');
    hminEl.textContent = 'H_min = —';
    ymaxEl.textContent = 'y_max (과수렴 최대충격량) = —';
    detEl.textContent  = '시뮬레이션 실행 후 계산됩니다.';
  }

  function runEpsDeltaAnalysis() {
    if (!features.epsDelta) return;
    const hminEl = $('eps-hmin'), ymaxEl = $('eps-ymax'), detEl = $('eps-detail');
    const eps  = +($('inp-eps').value) || 0.0001;
    const res  = calcHmin(eps);
    if (!res) { hminEl.textContent = 'H_min = 계산 불가 (파라미터 부족)'; return; }
    const { hMin, vt, g, tau } = res;

    hminEl.textContent = `H_min = ${hMin.toFixed(1)} m  (v_t = ${vt.toFixed(2)} m/s, τ = ${tau.toFixed(3)})`;

    // y_max 계산 (비동기적 느낌을 주지 않게 동기로, 경량 시뮬)
    const mass   = +inpMass.value, area = +inpArea.value, cd = +inpCd.value;
    const tempOff = +inpTemp.value || 0, hum = +inpHumidity.value || 50;
    const ym = findYmax(hMin, mass, area, cd, g, tempOff, hum);
    if (ym.reason) {
      ymaxEl.textContent = `y_max: ${ym.reason}`;
      detEl.textContent = `γ_eff·Δt = ${((2 * g / vt) * 0.05).toFixed(4)} ≤ 1 → 고정 Δt에서도 안정`;
    } else {
      ymaxEl.textContent = `y_max = ${ym.ymax.toFixed(1)} m  (충격속도 ${ym.vmaxImpact.toFixed(2)} m/s)`;
      const overshoot = ((ym.vmaxImpact / vt - 1) * 100).toFixed(2);
      detEl.textContent = `γ_eff·Δt = ${ym.gammaDt.toFixed(3)} > 1 | 종단속도 초과 ${overshoot}% | ε = ${eps}`;
    }
  }

  // ε 입력 변경 시 재계산
  const inpEps = $('inp-eps');
  inpEps.addEventListener('input', () => { if (features.epsDelta && simResult) runEpsDeltaAnalysis(); });
  ...
  // 실제 적분 로직은 physics-core.js의 runFallSimulation()(순수 함수, 전역 스크립트로 로드됨)에
  // 있으며, 메인 스레드(단일 물체)와 sim-worker.js(다중 물체 병렬)가 동일 코드를 공유한다.
  function buildSimParams(mass, area, cd) {
    return {
      mass, area, cd,
      h0: +inpHeight.value, v0: +inpV0.value, g: +inpG.value,
      wx: features.wind?(+inpWindX.value||0):0,
      wz: features.wind?(+inpWindZ.value||0):0,
      tempOff: +inpTemp.value||0, hum: +inpHumidity.value||50,
      // [F13] projectile decomposition
      launchAngle: features.projectile?(+inpLaunchAngle.value||45):90,
      launchAzimuth: features.projectile?(+inpLaunchAzimuth.value||0):0,
      // [F12] slope-adjusted gravity
      slopeDeg: (features.terrain&&selTerrain.value==='slope')?(+inpSlope.value||0):0,
      // [F7] magnus params
      omega: features.magnus?((+inpSpinRpm.value||0)*2*Math.PI/60):0,
      spinAxis: spinAxisVec(),
      atmosphereOn: features.atmosphere,
      heatOn: features.heat,
      adaptiveDt: features.adaptiveDt,
    };
  }
  function localSimulate(mass, area, cd) {
    return runFallSimulation(buildSimParams(mass, area, cd));
  }

  // ── [F11] 다중 물체 병렬 시뮬레이션 (Web Worker 풀) ──
  // 물체가 2개 이상일 때만 워커를 띄운다(1개면 스레드 전환 비용이 더 커서 이득이 없음).
  // 워커 생성/postMessage 실패 시(구형 환경 등) 자동으로 동기 계산으로 폴백한다.
  let simWorkerPool = [];
  function getSimWorker(i) {
    if (!simWorkerPool[i]) simWorkerPool[i] = new Worker('sim-worker.js');
    return simWorkerPool[i];
  }
  function simulateOnWorker(worker, params) {
  ...
  async function simulateMultiObjectsParallel(objects) {
    const paramsList = objects.map(o => buildSimParams(o.mass, o.area, o.cd));
    if (typeof Worker === 'undefined' || objects.length < 2) {
      return paramsList.map(p => runFallSimulation(p));
    }
    try {
      return await Promise.all(paramsList.map((p, i) => simulateOnWorker(getSimWorker(i), p)));
    } catch (err) {
      console.warn('[parallel-sim] worker 실패, 동기 계산으로 대체:', err);
      return paramsList.map(p => runFallSimulation(p));
    }
  }
  ...
  function resize3() {
    ...
    // 폭/높이가 0이면(레이아웃 전환 순간 등) aspect=w/h 가 0/Infinity/NaN 이 되어
    // 투영행렬이 깨지고 이후 아무것도 렌더되지 않는다(검은 화면). 유효할 때만 갱신.
    if (w < 1 || h < 1) return;
  ...
  // ── 모델 형상에서 항력계수(Cd) 추정 ──
  // 실제 Cd 는 CFD 로만 정확히 구할 수 있으므로, 여기서는 모델의 기하학적 특징으로
  // 물리적으로 타당한 범위(0.04~2.1)의 근사값을 계산한다. 낙하 방향은 수직(Y)으로 가정.
  //  1) 유선형 정도(fineness) = 낙하방향 길이 / √(정면 단면적)  → 길쭉할수록 Cd ↓
  //  2) 채움비(fillRatio) = 모델 부피 / 바운딩박스 부피          → 뭉툭/각질수록 Cd ↑ (구는 낮음)
  //  3) 정면 종횡비(frontalAspect)                                 → 납작·길쭉한 판일수록 Cd ↑
  // 모두 무차원(스케일 불변)이라 스케일 전 좌표로 계산해도 된다.
  function estimateCdFromModel(obj, size) {
    const sx = Math.max(size.x, 1e-6), sy = Math.max(size.y, 1e-6), sz = Math.max(size.z, 1e-6);
    obj.updateMatrixWorld(true);
    let area = 0, vol = 0, tris = 0;
    const vA = new THREE.Vector3(), vB = new THREE.Vector3(), vC = new THREE.Vector3();
    const ab = new THREE.Vector3(), ac = new THREE.Vector3(), cr = new THREE.Vector3();
    obj.traverse(c => {
      if (!c.isMesh || !c.geometry || !c.geometry.attributes || !c.geometry.attributes.position) return;
      const g = c.geometry, pos = g.attributes.position, idx = g.index, m = c.matrixWorld;
      ...
      for (let t = 0; t < triCount; t++) {
        ...
        area += 0.5 * cr.length();
        vol += vA.dot(cr) / 6.0;   // 원점 기준 부호부 사면체 부피(닫힌 메시면 합이 실제 부피)
        tris++;
      }
    });
    vol = Math.abs(vol);
    const bboxVol = sx * sy * sz;
    const fineness = sy / Math.sqrt(sx * sz);
    const fillRatio = (tris > 0 && bboxVol > 0) ? Math.min(1, Math.max(0.05, vol / bboxVol)) : 0.5;
    const frontalAspect = Math.max(sx, sz) / Math.max(Math.min(sx, sz), 1e-6);
    const baseCd   = Math.min(1.3, Math.max(0.1, 0.18 + 1.0 / (1 + fineness * fineness)));
    const bluntMul = 0.75 + 0.55 * fillRatio;                        // 0.05→0.78, 0.5→1.03, 1→1.30
    const aspectMul= Math.min(1.4, 1 + 0.06 * (frontalAspect - 1));  // 살짝만 반영
    const cd = Math.min(2.1, Math.max(0.04, baseCd * bluntMul * aspectMul));
  ...
  function applyGlbToFalling(obj) {
    ...
    // 모델 형상으로부터 항력계수(Cd)를 추정해 입력칸에 자동 설정(사용자가 이후 수정 가능).
    try {
      const cdEst = estimateCdFromModel(obj, size);
      if (isFinite(cdEst) && inpCd) {
        inpCd.value = cdEst;
        inpCd.dispatchEvent(new Event('input', { bubbles: true }));
        updateTV();
        toast(`모델 형상에서 Cd ≈ ${cdEst} 로 자동 추정됨 (필요시 수정 가능)`, 'info', 3500);
      }
    } catch (e) { console.warn('[Cd estimate] 실패:', e); }
  ...
  function loadModelFromPath(path, label) {
    ...
    if (!loader) {
      toast(`${kind} loader unavailable — model feature disabled.\n` +
            (glbLoaderError ? String(glbLoaderError.message || glbLoaderError) : '') +
            '\n→ Check import map / node_modules/three installation.', 'error', 9000);
      return;
    }
    const seq = ++glbLoadSeq;
    glbLoading = true;
    toast(`Loading ${kind}… (${name})`, 'info', 2000);
    loader.load(
      path,
      result => {
        ...
        toast(`${kind} loaded: ${name}`, 'ok', 2500);
      },
      undefined,
      err => {
        if (seq !== glbLoadSeq) return;
        ...
        toast(`${kind} load failed: ${name}\n${err && (err.message || err.type) || err}\nPath: ${path}`, 'error', 9000);
      }
    );
  }
  // 드롭다운 선택 + 물리값 자동 설정
  ...
  async function loadLocalModelFile(f, btn) {
    ...
    if (!loader) {
      toast(`${kind} loader unavailable — cannot open ${kind} files.`, 'error', 9000);
      return;
    }
    ...
    toast(`Loading ${kind}… (${f.name})`, 'info', 2000);
    loader.load(
      url,
      result => {
        ...
        glbLoading = false;
        applyGlbToFalling(kind === 'STL' ? makeStlMesh(result) : result.scene);
        toast(`${kind} loaded: ${f.name}`, 'ok', 2500);
      },
      undefined,
      err => {
        URL.revokeObjectURL(url);
  ...
  function triggerThermalFailure(frame, yVisual){
    ...
    atmBadge.textContent = mode==='burnup' ? 'BURNED UP' : 'DISINTEGRATED';
  ...
  function renderMoList() {
    ...
    moStaggerRow.style.display=moObjects.length>0?'flex':'none';
    moObjects.forEach((o,i)=>{
      ...
      div.innerHTML=
        ...
        `<div class="field"><label>낙하 시간차 <span>s</span></label><input type="number" class="mo-f" data-k="dropDelay" data-i="${i}" value="${o.dropDelay||0}" min="0" max="60" step="0.1"></div>`+
        `</div>`;
  ...
  btnAddObj.addEventListener('click',()=>{
    ...
    moObjects.push({mass:2*(i+1),area:0.02,cd:0.47,dropDelay:0,result:null,mesh,color:MO_COLORS[i],glbMesh:null,glbPath:''});
    renderMoList(); requestRender();
  });
  btnAutoStagger.addEventListener('click',()=>{
    const step=Math.max(0,+inpStaggerStep.value||0);
    moObjects.forEach((o,i)=>{ o.dropDelay=i*step; });
    renderMoList();
  });
  ...
  function buildTable(result) {
    ...
    for(let i=0;i<frames.length;i++){
      ...
      // % TERMINAL = 현재 속도 / (상단에 표시되는) 해수면 기준 종단속도.
      // 고도가 높아 공기가 희박하면 실제 속도가 이 종단속도를 넘어설 수 있으므로 100%를 초과할 수 있다
      // (막대 그래프만 100%에서 포화). 이렇게 해야 상단의 TERMINAL VELOCITY 값과 일관된다.
      const pct=vt>0?Math.abs(f.v)/vt*100:0;
  ...
  async function exportXLSX(){
    ...
    const rows=simResult.frames.map(f=>{
      // 상단 표시 종단속도(해수면 기준) 대비 %. 고고도에서 100% 초과 가능.
      const pct=vt>0?Math.abs(f.v)/vt*100:0;
      const row=[
  ...
  function exportCSV(){
    ...
    // [F11] 다중 물체 요약 — 각 물체의 시간차/충돌 시각을 별도 표로 CSV 하단에 추가
    let moSection='';
    if(features.multiobj && moObjects.some(o=>o.result)){
      moSection='\n\nMulti-Object Summary\n'+
        'Object,Mass(kg),Area(m2),Cd,DropDelay(s),TerminalVel(m/s),ImpactVel(m/s),FallTime(s),LandingTime(s)\n'+
        moObjects.filter(o=>o.result).map((o,i)=>{
          const delay=o.dropDelay||0;
          return `Object ${i+2},${o.mass},${o.area},${o.cd},${delay.toFixed(2)},`+
                 `${o.result.terminalVelocity.toFixed(3)},${Math.abs(o.result.impactVelocity).toFixed(3)},`+
                 `${o.result.fallTime.toFixed(2)},${(o.result.fallTime+delay).toFixed(2)}`;
        }).join('\n');
    }
    const blob=new Blob(['﻿'+header+'\n'+rows.join('\n')+moSection],{type:'text/csv;charset=utf-8'});
    const link=document.createElement('a');
  ...
  function updateOfflineBadge(){
    ...
    if(features.offline){
      ...
      lastCloudInfo={status:'오프라인 모드 — 클라우드 저장 불가',url:null};
      cloudStatus.textContent=lastCloudInfo.status; cloudUrl.style.display='none';
    } else {
      badge.style.display='none';
      lastCloudInfo={status:'대기 중',url:null};
      cloudStatus.textContent=lastCloudInfo.status;
    }
  }
  async function saveToCloud(){
    if(features.offline){toast('오프라인 모드가 활성화되어 있습니다.\n오프라인 모드를 끄고 다시 시도하세요.','error');return;}
    if(!simResult){toast('시뮬레이션 결과가 없습니다. Run 후 시도하세요.','error');return;}
    lastCloudInfo={status:'저장 중…',url:null};
    cloudStatus.textContent=lastCloudInfo.status;cloudUrl.style.display='none';
    btnSaveCloud.disabled=true;
    try{
      await _authReady;
      if(!_auth.currentUser) throw new Error('로그인(익명 인증)에 실패했습니다.');
      const tgt=targetObjects[+selTarget.value];
      const counterRef=doc(_db,'meta','counter');
      let newId='';
      await runTransaction(_db,async tx=>{
        const snap=await tx.get(counterRef);
        const n=(snap.exists()?snap.data().nextId||1:1);
        newId=String(n).padStart(3,'0');
        tx.set(counterRef,{nextId:n+1},{merge:true});
        const resultRef=doc(_db,'results',newId);
        tx.set(resultRef,{
          id:newId,
          ...
          summary:{
            terminalVelocity:simResult.terminalVelocity||0,
            impactVelocity:simResult.impactVelocity||0,
            fallTime:simResult.fallTime||0,
            impactEnergy:simResult.impactData?simResult.impactData.impactEnergy:0,
            destructionLevel:simResult.impactData?simResult.impactData.destructionLevel:'—',
            destructionRatio:simResult.impactData?simResult.impactData.destructionRatio:0,
            drift:Math.hypot(simResult.driftX||0,simResult.driftZ||0),
          },
          frames:simResult.frames,
        });
      });
      const url=`https://sp202606-web.pages.dev/results/${newId}`;
      lastCloudInfo={status:`저장 완료 → #${newId}`, url};
      cloudStatus.textContent=lastCloudInfo.status;
      cloudUrl.textContent=url;cloudUrl.style.display='block';
      cloudUrl.onclick=()=>window.appBridge.openExternal(url);
      toast(`클라우드 저장 완료! #${newId}\n${url}`,'ok',6000);
    }catch(err){
      console.error('[saveToCloud]',err);
      lastCloudInfo={status:'저장 실패: '+err.message, url:null};
      cloudStatus.textContent=lastCloudInfo.status;
      toast('클라우드 저장 실패: '+err.message,'error');
    }finally{
  ...
  function refreshLsPresets() {
    ...
    selLs.innerHTML = '<option value="">Saved presets...</option>' +
  ...
  // 프리셋 파일 내보내기/가져오기 (JSON) — localStorage 외에 파일로 백업/공유 가능
  btnLsExport && btnLsExport.addEventListener('click', () => {
    const presets = getLsPresets();
    if (Object.keys(presets).length === 0) { toast('내보낼 저장된 프리셋이 없습니다.', 'info', 3000); return; }
    const blob = new Blob([JSON.stringify(presets, null, 2)], { type: 'application/json' });
    const link = document.createElement('a');
    link.download = 'physics-sim-presets.json'; link.href = URL.createObjectURL(blob);
    link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 1000);
  });
  btnLsImport && btnLsImport.addEventListener('click', () => inpLsImportFile.click());
  inpLsImportFile && inpLsImportFile.addEventListener('change', () => {
    const file = inpLsImportFile.files && inpLsImportFile.files[0];
    inpLsImportFile.value = '';
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      try {
        ...
        Object.assign(presets, imported); // 같은 이름이면 가져온 값으로 덮어씀
        localStorage.setItem(LS_KEY, JSON.stringify(presets));
        refreshLsPresets();
        toast(`프리셋 ${count}개를 가져왔습니다.`, 'info', 3000);
      } catch (err) {
        toast('프리셋 파일을 읽을 수 없습니다 (JSON 형식 오류).', 'error', 4000);
      }
    };
  ...
  btnRun.addEventListener('click',async()=>{
    ...
    if(selShape.value==='custom'){
      if(glbLoading) toast('GLB still loading — run again in a moment to use the model.', 'info', 3500);
      else if(!glbMesh) toast('Custom (GLB) mode: no model selected. Pick a preset or load a file first. (Falling as default sphere)', 'error', 6000);
    }
    ...
    // [F11] extra objects — Web Worker 풀로 병렬 계산(물체 2개 이상일 때)
    if(features.multiobj && moObjects.length>0){
      const moResults=await simulateMultiObjectsParallel(moObjects);
      moObjects.forEach((o,i)=>{ o.result=moResults[i]; o._cs={c:0}; });
    }
    ...
    // [F-ADT] adaptive dt 통계 업데이트
    updateAdaptiveDtDisplay(result);

    // [ε-δ] ε-δ 분석 자동 업데이트
    if (features.epsDelta) runEpsDeltaAnalysis();
  ...
  function buildResultData(thermalFail){
    ...
    return {
      ...
      cloudStatus: lastCloudInfo.status,
      cloudUrl: lastCloudInfo.url,
    };
  }
  ...
  function applyViewLayout(){
    ...
    // window 'resize' 리스너는 resize3() 를 호출하지 않으므로(그래프만 다시 그림)
    // 3D 캔버스 리사이즈는 resize3() 를 직접 호출한다. 브라우저가 새 레이아웃을
    // 반영한 뒤 크기를 읽도록 requestAnimationFrame 두 번(레이아웃 확정)으로 지연.
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      resize3();
      if(simResult) drawGraph(activeTab);
      requestRender();
  ...
  if(window.appBridge){
    ...
    if(window.appBridge.onFilePicked) window.appBridge.onFilePicked(p=>{
      ...
      }catch(e){ toast('File load failed: '+(e&&e.message||e), 'error', 6000); }
    });
    ...
    window.appBridge.onResultsAction(a=>{
      ...
      else if(a.action==='cloud'){
        // 결과창에서 클라우드 저장을 요청한 경우: 즉시 "저장 중" 상태를 보여주고,
        // 완료되면 최종 상태(완료/실패)와 링크를 다시 push한다.
        const p = saveToCloud();
        if(window.appBridge) window.appBridge.showResults(buildResultData(thermalFailed));
        p.then(()=>{ if(window.appBridge) window.appBridge.showResults(buildResultData(thermalFailed)); });
      }
    });
  ...
  function animLoop(now){
    ...
    if(playing&&simResult){
      ...
      const moMaxTime=features.multiobj&&moObjects.some(o=>o.result)
        ?Math.max(...moObjects.filter(o=>o.result).map(o=>o.result.fallTime+(o.dropDelay||0))):0;
      ...
      // [F11] animate extra objects (시간차 낙하: dropDelay 초만큼 대기 후 낙하 시작)
      if(features.multiobj){
        moObjects.forEach((o,i)=>{
          if(!o.result||!o.mesh)return;
          const ox0=(i+1)*5;
          const objTime=playHead-(o.dropDelay||0);
          if(objTime<0){
            // 아직 낙하 시작 전 — 초기 높이에서 대기
            o.mesh.position.set(ox0, baseY+visualH, 0);
            if(o.glbMesh) o.glbMesh.position.set(ox0, baseY+visualH, 0);
            return;
          }
          if(!o._cs)o._cs={c:0};
          const fr2=lerpFrame(o.result.frames,objTime,o._cs);
          const p2=Math.max(0,Math.min(1,fr2.h/currentH0));
          const ox=ox0+(fr2.px||0)*0.05;
      ...
      // [F11] 물체간 충돌(시각 효과) — 겹치면 서로 밀어내기만 함(핵심 물리/충돌 결과는 그대로 유지)
      if(features.multiobj && features.mocollision){
        const bodies=[];
        if(!impacted && fallingMesh && fallingMesh.visible) bodies.push({mesh:fallingMesh,glb:glbMesh,r:1.0});
        moObjects.forEach(o=>{ if(o.result && o.mesh && o.mesh.visible) bodies.push({mesh:o.mesh,glb:o.glbMesh,r:1.0}); });
        for(let bi=0; bi<bodies.length; bi++){
          for(let bj=bi+1; bj<bodies.length; bj++){
            const a=bodies[bi].mesh.position, b=bodies[bj].mesh.position;
            const dx=b.x-a.x, dz=b.z-a.z;
            const dist=Math.hypot(dx,dz);
            const minDist=bodies[bi].r+bodies[bj].r;
            if(dist>1e-6 && dist<minDist){
              const push=(minDist-dist)/2, nx=dx/dist, nz=dz/dist;
              a.x-=nx*push; a.z-=nz*push;
              b.x+=nx*push; b.z+=nz*push;
            } else if(dist<=1e-6){
              a.x-=minDist/2; b.x+=minDist/2;
            }
            if(bodies[bi].glb){bodies[bi].glb.position.x=a.x;bodies[bi].glb.position.z=a.z;}
            if(bodies[bj].glb){bodies[bj].glb.position.x=b.x;bodies[bj].glb.position.z=b.z;}
          }
        }
      }
      ...
      if(playHead>=mainFallTime&&!impacted){
        ...
        if(withstood){
          atmBadge.textContent='WITHSTOOD'; atmBadge.style.color='#3fb950';
    ...
    if(bouncing&&fallingMesh){
      ...
      // 회전 속도를 바운스 속도(잔여 에너지)에 비례해 감쇠시켜, 착지 후 거의 멈춘
      // 상태에서도 계속 빠르게 도는 것처럼 보이는 문제를 방지한다.
      const spinDamp=Math.min(1,Math.abs(bounceVel)/3);
      fallingMesh.rotation.x+=0.06*spinDamp;
      if(glbMesh){glbMesh.position.y=bounceY+glbGroundOffset();glbMesh.rotation.y+=0.06*spinDamp;}
      if(camFollowMode){
  ...
  if (!gltfLoader) {
    setTimeout(() => toast(
      'GLB loader failed — GLB model selection is disabled (rendering works normally).\n' +
      (glbLoaderError ? String(glbLoaderError.message || glbLoaderError) : '') +
      '\n→ Try restarting or reinstalling the app.', 'error', 9000), 800);
  }
})();
