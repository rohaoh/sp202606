...
| F-ADT | 적응형 Δt | ON | 드래그 강직성에 따라 타임스텝 자동 조정 (과수렴 방지) |
| F-EPS | ε-δ 수렴 분석 | OFF | 입실론-델타 논법으로 H_min·y_max 자동 계산 |
...
## 2026-7-7(1) 변경 사항 — ε-δ 수렴 분석 (F-EPS)

`2026-7-7` 기반. **C++ 물리 코드 변경 없음 — `npm run build-addon` 불필요.**

### 기능 개요

좌측 패널 "**ε-δ Analysis**" 섹션 토글 ON 후 시뮬레이션을 실행하면 자동으로 두 값을 계산합니다.

#### H_min — 최소 낙하 높이 (ε-δ 논법)

입실론-델타 논법의 수렴 정의를 낙하 물리에 적용합니다:

```
lim(H → ∞) v_impact(H) = v_t
∀ε > 0, ∃H_min : H ≥ H_min → |v_t − v_impact| < ε · v_t
```

**해석적 공식** (1D, 해수면 일정 밀도, 수직 낙하 가정):

| 단계 | 수식 |
|------|------|
| 종단속도 | `v_t = √(2mg / ρCdA)` |
| 도달 조건 | `v(T) = (1−ε)·v_t` → `tanh(gT/v_t) = 1−ε` |
| 역함수 | `τ = atanh(1−ε) = ½·ln((2−ε)/ε)` |
| **최소 높이** | **`H_min = (v_t²/g) · ln(cosh(τ))`** |

ε = 0.0001(기본값) 예시: 볼링볼 → H_min ≈ 1807 m, 인체 → H_min ≈ 4100 m

#### y_max — 과수렴 최대 충격량 높이

고정 Δt = 0.05 s 오일러 적분에서 γ·Δt > 1 이면 속도가 v_t 를 중심으로 진동(과수렴). 높이에 따라 충돌 직전 위상이 달라 충격량 J = m·v_impact 에 극대값이 생깁니다.

수치 탐색: H_min 주변을 200점 스캔 → 황금 탐색(40회)으로 정밀화.

γ·Δt ≤ 1 이면 과수렴이 없으므로 y_max 는 "의미 없음"으로 표시합니다.

### UI 위치

좌측 패널 > **ε-δ Analysis** 섹션 (Adaptive Δt 바로 아래)

- ε 입력칸으로 허용 오차 변경 가능 (기본 0.0001 = 99.99%)
- 시뮬레이션 실행마다 자동 재계산

---

## 2026-7-7 변경 사항 — 적응형 타임스텝(Adaptive Δt) 구현

`2026-7-6(9)` 기반. **C++ 물리 코드 변경 → `npm run build-addon` 재실행 필요.**

### 과수렴(Over-convergence) 문제 분석 및 해결

자유낙하 중 고도가 낮아지면 공기 밀도가 급증(예: 80km ≈ 10⁻⁵ kg/m³ → 해수면 1.225 kg/m³),
드래그 강직성 γ·Δt 가 1을 넘는 순간 속도가 단말속도를 중심으로 진동(과수렴)하는 수치 불안정 발생.

**불안정 조건**: `γ·Δt ≥ 1`, where `γ = ρ·Cd·A·|v| / (2m)` (유효 드래그 감쇠율)

예시: 볼링볼(7 kg) v₀ = 9000 m/s, 해수면 고도 100m → γ·Δt = 1.06 (임계 초과)  
- 고정 Δt = 0.05s: 속도가 +9000 → −533 m/s로 반전(에너지 소산 오류) → 충격 속도 65 m/s (잘못된 값)  
- 적응형 Δt = 0.001–5.6ms: 물리적으로 올바른 충격 속도 6930 m/s

### 구현 내용

**`physics/physics.cpp`** (`SimInput.adaptiveDt` 필드 추가):
- 안정성 기준: `dt_stab = 0.5 / γ`
- 고도 정밀도 기준: `dt_alt = min(altitude × 0.5, 200m) / |vy|` (지면 통과 방지)
- 선택: `dt = clamp(min(dt_stab, dt_alt), 0.001s, 0.1s)`
- `ImpactResult`에 `minDtUsed`, `maxDtUsed`, `totalSteps` 통계 추가
- `PhysicsFrame`에 `dtUsed` 필드 추가
- 적응형 모드에서 프레임은 0.05s 간격(+ 충돌 직전)만 기록해 메모리 보호

**`physics-core.js`** (JS 동일 로직):
- 동일한 적응형 dt 알고리즘 구현
- `runFallSimulation()` 반환값에 `minDt`, `maxDt`, `totalSteps` 추가

**`index.html`**: Adaptive Δt 토글 섹션 추가 (원리 설명 + 실시간 통계 표시)

**`renderer.js`**: `features.adaptiveDt` 토글, `updateAdaptiveDtDisplay()` 함수 추가

---

## 2026-7-6(9) 변경 사항 (배포 버전 3.2.2로 업데이트)

`2026-7-6(8)` 기반. 앱 코드 변경 없음 — `npm run build-addon` 재실행 불필요.

- `package.json`의 `version`을 `3.2.2`로 올려, `2026-7-6(8)`의 Cloud Save 익명 인증
  추가분을 포함한 새 릴리즈(`v3.2.2`)를 배포할 수 있게 함.
- `RELEASE_NOTES_LATEST.md`를 v3.2.1 → v3.2.2 변경 사항으로 갱신.
- 배포 워크플로우(`/.github/workflows/release.yml`)의 `PROJECT_DIR`을 이 폴더로 갱신.

---

## 2026-7-6(8) 변경 사항 (Cloud Save "missing or insufficient permissions" 수정 — 익명 인증 추가)

`2026-7-6(7)` 기반. 앱 코드 변경 없음 — `npm run build-addon` 재실행 불필요.

### 원인
- CSP를 고쳐 Firestore로 네트워크 요청 자체는 나가게 됐지만(2026-7-6(6)), 앱에 로그인
  처리가 전혀 없어 항상 **비로그인 상태**로 Firestore에 쓰기를 시도하고 있었음.
  Firestore 보안 규칙이 `request.auth != null`을 요구하면 정확히 "missing or
  insufficient permissions" 오류가 남.

### 수정
- `firebase/auth`를 추가해 앱 시작 시 조용히 **익명 로그인**(`signInAnonymously`)을
  수행(로그인 화면 없음, 사용자는 인지하지 못함).
- `saveToCloud()`가 Firestore에 쓰기 전에 익명 로그인이 끝날 때까지 기다리도록 함.
- CSP는 이미 `https://*.googleapis.com`을 허용해뒀어서(2026-7-6(6)) 추가 변경 불필요
  (Firebase Auth가 쓰는 `identitytoolkit.googleapis.com`/`securetoken.googleapis.com`도
  이 와일드카드에 포함됨).

### ⚠️ Firebase 콘솔에서 별도로 해야 할 작업
이 수정은 앱이 "인증된 사용자"로 요청하도록 만드는 것뿐이고, **Firestore 보안 규칙
자체는 Firebase 콘솔에서 직접 설정해야 함**(이 저장소에서 관리하지 않음). 아래처럼
바꿔야 함:
```
rules_version = '2';
...
```
(Firebase 콘솔 → Firestore Database → Rules 탭에서 붙여넣기)

---

## 2026-7-6(7) 변경 사항 (배포 버전 3.2.1로 업데이트)

`2026-7-6(6)` 기반. 앱 코드 변경 없음 — `npm run build-addon` 재실행 불필요.

- `package.json`의 `version`을 `3.2.1`로 올려, `2026-7-6(6)`의 Cloud Save
  "connection failed" 버그 수정분을 포함한 새 릴리즈(`v3.2.1`)를 배포할 수 있게 함.
- `RELEASE_NOTES_LATEST.md`를 v3.2.0 → v3.2.1 변경 사항으로 갱신.
- 배포 워크플로우(`/.github/workflows/release.yml`)의 `PROJECT_DIR`을 이 폴더로 갱신
  (직전 `2026-7-6(6)` 커밋에서 이 갱신을 빠뜨렸던 것도 함께 바로잡음).

---

## 2026-7-6(6) 변경 사항 (Cloud Save "connection failed" 버그 수정 — CSP가 Firestore 접속을 막고 있었음)

`2026-7-6(5)` 기반. 앱 코드 변경 없음 — `npm run build-addon` 재실행 불필요.

### 원인
- `index.html`의 CSP(Content-Security-Policy)에 `connect-src`가 따로 지정돼 있지 않아
  `default-src 'self'`를 그대로 물려받았음. 그 결과 `fetch`/`XHR`/WebSocket으로 나가는
  네트워크 요청이 전부 **같은 origin(로컬 dev 서버)으로만 제한**되어, Cloud Save가
  Firestore(`firestore.googleapis.com`)와 통신하려는 시도 자체가 브라우저(Chromium)
  단에서 CSP로 차단되고 있었음 — 그래서 저장을 시도하면 "connection failed"처럼 보이는
  에러가 났던 것.

### 수정
- CSP에 `connect-src 'self' https://*.googleapis.com https://*.firebaseio.com
  wss://*.firebaseio.com;`을 추가해 Firebase Firestore가 실제로 통신하는 호스트를
  명시적으로 허용. 결과창(`windows/results.html`) 등 Firebase를 쓰지 않는 다른 창들은
  CSP 변경 없이 그대로 둠(불필요한 네트워크 허용 범위 확대 방지).

`2026-7-6(4)` 기반. 앱 코드 변경 없음 — `npm run build-addon` 재실행 불필요.

### 문제
- 직전 배포 시도(2026-7-6(4) 기준 워크플로우 실행)가 **모든 잡이 "성공"으로 끝났는데도
  실제로는 설치파일이 하나도 업로드되지 않았음.** 원인은 `package.json`의 `version`이
  여전히 `3.0.0`이었고, `v3.0.0` 태그에는 이미 2026-7-2에 **정식 발행(비-draft)된 릴리즈**가
  존재했던 것. electron-builder는 안전장치로 이미 발행된(비-draft) 릴리즈에는 새 자산을
  올리지 않고 조용히 스킵함(`reason=existing type not compatible with publishing type`).
- 릴리즈 노트(`RELEASE_NOTES_LATEST.md`)도 v3.0.0 시점 그대로 멈춰 있어서, 이후 진행된
  물리 버그 수정·XLSX 버그 수정·Cloud Save·배포 파이프라인 변경 내용이 전혀 반영돼
  있지 않았음.

### 수정
- `package.json`의 `version`을 `3.2.0`으로 올림 — 존재하지 않는 새 태그(`v3.2.0`)이므로
  electron-builder가 정상적으로 새 draft 릴리즈를 만들고 x64/arm64 설치파일을 올릴 수 있음.
- `RELEASE_NOTES_LATEST.md`를 v3.0.0 → v3.2.0 사이의 실제 변경 사항(물리 버그 수정,
  XLSX 버그 수정, Cloud Save, 배포 파이프라인 x64/arm64화)으로 갱신.
- 배포 워크플로우(`/.github/workflows/release.yml`)의 `PROJECT_DIR`을 이 폴더로 갱신.

`2026-7-6(3)` 기반. 배포 자동화(`/.github/workflows/release.yml`, 저장소 루트)만 변경.
앱 코드 변경 없음 — `npm run build-addon` 재실행 불필요.

### 문제
- 배포 워크플로우의 `PROJECT_DIR`이 오래전 폴더(`codes/2026-7-1(4)`)에 그대로 고정돼 있어,
  이 상태로 배포하면 그 이후의 모든 수정(물리 버그 수정, XLSX 버그 수정, Cloud Save 등)이
  전부 빠진 옛날 버전이 배포될 뻔했음. 최신 폴더(`codes/2026-7-6(4)`)로 갱신.
- ARM64 빌드가 아예 없었음(x64 전용) — `package.json`의 `cmake-js.arch`도 `x64`로 고정.

### 변경
- 배포 워크플로우에 **x64 / arm64 매트릭스**를 추가해 두 아키텍처를 순차로 빌드
  (`max-parallel: 1` — 같은 GitHub Release에 동시에 자산을 올리다 충돌하는 것을 방지).
  MSVC 크로스 컴파일 도구는 `ilammy/msvc-dev-cmd@v1`의 `amd64_arm64` 옵션으로 설정하고,
  네이티브 addon은 `cmake-js compile --arch=<arch>`로 아키텍처별로 각각 컴파일.
- **아키텍처별 제작자(publisher) 지정**: electron-builder 실행 직전에
  `npm pkg set author="..."`로 `package.json`의 author를 그때그때 바꿔치기해서,
  **x64는 "오로하", arm64는 "서예준"**이 각 설치파일(exe/msi)의 제작자로 기록되도록 함.
- `package.json`의 `build.nsis`/`build.msi`에 `artifactName`을 `${arch}` 포함 형태로
  지정 — 그렇지 않으면 x64/arm64 설치파일이 같은 파일명으로 만들어져 같은 릴리즈 안에서
  서로 덮어쓰게 됨.
- 릴리즈 노트 반영 + draft 해제(정식 발행)는 두 아키텍처 자산이 모두 올라간 뒤 별도의
  `finalize` 잡에서 한 번만 실행하도록 분리(매트릭스 잡이 각자 발행을 시도해 레이스
  컨디션이 생기는 것을 방지).

> 참고: 이 저장소에서 ARM64 크로스 컴파일을 시도하는 것은 이번이 처음이라, 첫 실행에서
> 네이티브 addon 컴파일 단계가 실패할 가능성이 있음. 실패 시 Actions 로그를 보고 후속
> 조치가 필요할 수 있음.

`2026-7-6(2)` 기반. Cloud Save 기능을 더 이상 토글로 켜고 꺼야 하는 게 아니라 항상 사용
가능하게 바꾸고, 결과창(별도 팝업)에서도 클라우드 저장과 링크 생성을 바로 할 수 있게 함.

### 1. Cloud Save 토글 제거 (항상 켜짐)
- 상단 툴바의 **☁ CLOUD** 버튼이 이제 토글 없이 항상 보임(기존엔 설정 패널에서
  "Cloud Save" 토글을 켜야만 버튼이 나타났음).
- 좌측 설정 패널의 "Cloud Save" 섹션에서 켜고 끄는 토글 스위치를 제거하고, 상태/링크
  표시만 남김(항상 펼쳐진 상태).
- `features.cloud` 플래그와 관련 분기(`applyFeature('cloud')` 등)를 모두 제거해 코드도
  같이 정리함.

### 2. 결과창에서 클라우드 저장 + 링크 생성 지원
- 결과창(`windows/results.html`)에 **☁ Save to Cloud** 버튼과 상태/링크 표시 영역을
  추가. 버튼을 누르면 메인 창의 `saveToCloud()`가 그대로 실행되고("저장 중…" →
  "저장 완료 → #번호"), 완료되면 결과창에도 같은 상태와 클릭 가능한 링크가 표시됨.
- 결과창은 별도 팝업 창(별도 프로세스 렌더러)이라 메인 창의 DOM에 직접 접근할 수
  없으므로, 기존 결과 데이터 전달 채널(`results:action` → 메인 렌더러 → `results:show`)을
  그대로 재사용: 결과창 버튼 클릭 → `saveToCloud()` 실행 → 완료 시 결과 데이터를
  (클라우드 상태/링크 필드 포함해서) 결과창에 다시 push.
- **부수 버그 수정**: 클라우드 링크를 누르면 브라우저로 열리는 기능이, 실제로는
  `require('electron').shell.openExternal(...)`을 렌더러(`contextIsolation:true,
  nodeIntegration:false`)에서 직접 호출하고 있어 원래부터 동작하지 않고 콘솔에
  `require is not defined` 오류만 났었음(눌러도 아무 반응 없이 조용히 실패).
  `preload.js`/`windows/results-preload.js`에 `openExternal(url)` IPC 브릿지를 추가하고,
  `main.js`에서 우리가 만든 `sp202606-web.pages.dev` 링크인지 화이트리스트로 검증한 뒤
  `shell.openExternal()`을 호출하도록 수정 — 메인 창/결과창 양쪽 링크 클릭이 모두
  정상적으로 브라우저를 여는 것으로 고쳐짐.

---

## 2026-7-6(2) 변경 사항 (충격 가속도의 무게 의존성 추가 · 대기 밀도 지수 감소 모델로 정리)

`2026-7-6(1)` 기반. 두 가지 요청 반영: (1) 충돌 시 감속(가속도) 임계점이 무게에 따라
달라지도록 수정, (2) 고도별 대기 밀도가 지수함수적으로 감소하도록 공식 정리.

### 1. 충격량은 무게에 따라 달라지는데, 충돌 시 감속은 그렇지 않던 문제
- `calcImpact()`의 충돌 시간(`collisionTime`)이 오직 타겟의 강도(`yieldStrength`)와
  충돌 속도로만 정해지고 있었음. 그 결과 평균 충격력은 운동량(질량×속도)을 통해
  무게에 따라 달라졌지만, **충돌 중 평균 감속도(가속도) 자체는 무게와 무관하게 항상
  동일**했음 — 볼링공이든 소형차든 같은 속도로 부딪히면 감속 프로파일이 똑같았던 것.
  실제로는 무겁고/큰 충돌체일수록 접촉 중 더 많은 재질이 압축·변형되며 충돌 펄스가
  길어져(볼링공이 골프공보다 부드럽고 긴 충돌을 겪는 이유) 감속도가 낮아진다.
- 기준 질량 10kg 대비 `(mass/10)^0.12` 배율을 `collisionTime`에 곱하도록 수정. 지수를
  작게 잡아(0.12) 총 충격력은 여전히 질량에 따라 크게 늘어나되(운동량 효과가 주),
  무게가 감속도 자체에도 실제로 영향을 주도록 보정만 추가하는 수준으로 제한함.
  (예: 500kg 쇠구슬 vs 나무판 — 이전엔 볼링공과 동일한 감속 프로파일로 계산되던 것이,
  이제 더 길게 늘어난 충돌 펄스를 반영해 파괴 정도가 살짝 완화됨. 반대로 극단적으로
  무거운 커스텀 질량을 입력해도 충돌시간이 무한정 늘어나지 않도록 clamp 상한을
  0.02→0.05초로 소폭 확장.)

### 2. 대기 밀도가 고도에 따라 지수함수적으로 감소하도록 정리
- 기존에는 대류권(0~11km)과 하부 성층권 일부(20~32km) 구간에서 온도 선형 변화를 반영한
  **거듭제곱(power-law) 형태의 기압 공식**을 쓰고 있어, 등온층(11~20km, 32km 이상)의
  지수 감소 구간과 섞여 구간별로 "감소하는 모양"이 조금씩 달라 보였음.
  (수치적으로는 이미 연속·단조감소였고 버그는 아니었지만, 요청대로 전 구간이
  일관되게 지수함수 형태로 보이도록 정리함.)
- 각 층의 스케일 하이트(H = R·T/g)를 이용한 `exp(-h/H)` 형태로 대류권·20~32km 구간의
  기압 공식을 바꾸고, 층 경계에서 이전 층 값을 이어받아 연속성을 유지하도록 함(경계에서
  값이 끊기지 않음을 직접 검증). 온도(T) 자체의 층별 변화(대류권 선형 감소, 성층권
  등온/역전)는 그대로 유지 — 습도 보정 등 다른 계산에 영향 없음.
- **부수 수정**: 이 김에 JS 폴백(`physics-core.js`, 멀티 오브젝트 Web Worker 계산용)과
  UI 실시간 힌트(`renderer.js`의 `airDensityJS`)에도 동일하게 반영. 그 과정에서
  32~80km 구간을 JS 쪽만 스케일 하이트 10000m 하나로 뭉뚱그려 C++ 버전과 값이 어긋나던
  기존 불일치도 함께 발견해 맞춤(32~47km/47~80km 2단 구성으로 통일). 이제 세 구현
  (`physics.cpp`/`physics-core.js`/`renderer.js`) 모두 전 고도 구간에서 동일한 값을
  반환함을 직접 대조해 확인.

> **C++ 물리 코드(`physics/physics.cpp`)를 수정했습니다. 반드시 `npm run build-addon` 재실행** 후 실행하세요.

---

## 2026-7-6(1) 변경 사항 (XLSX 내보내기 시 "복구된 레코드" 경고 뜨던 버그 수정)

`2026-7-6` 기반. 궤적 데이터를 XLSX로 내보낸 뒤 Excel에서 열면
"복구된 레코드: /xl/worksheets/sheet1.xml 부분의 셀 정보" 경고가 뜨고 복구를
눌러야 열리던 문제 수정.

### 원인
- `main.js`의 `buildXlsxWithCharts()`는 라이브러리 없이 OOXML(zip+XML)을 직접 조립하는데,
  `xl/_rels/workbook.xml.rels`에 워크시트(`worksheet`) relationship만 나열하고
  **`xl/styles.xml`에 대한 relationship 선언이 통째로 빠져 있었음.**
- OOXML 스펙(ECMA-376)상 `<workbook>` 엘리먼트는 styles 파트를 r:id로 직접 가리키지
  않고, Excel은 오직 workbook의 relationship 파일에서 `Type=".../styles"`인 항목을 찾아
  스타일 파트를 로드한다. 이 relationship이 없으면 헤더 셀에 지정한 `s="1"`(볼드 스타일)
  같은 스타일 인덱스가 참조할 곳을 잃어, Excel이 열 때 해당 셀 정보를 복구/폐기하고
  경고를 띄우게 됨. `[Content_Types].xml`에는 styles.xml이 정상 등록돼 있어서 더 헷갈리는
  버그였음(파일 자체는 있는데 "연결"이 안 되어 있던 것).

### 수정
- `xl/_rels/workbook.xml.rels`에 `styles.xml`을 가리키는 relationship 항목을 추가
  (기존 워크시트들이 쓰는 rId와 겹치지 않는 새 rId 사용).
- 숫자 셀 작성 부분(`<c><v>${v}</v></c>`)에 `Number.isFinite()` 가드를 추가 —
  드물게 물리 계산값이 `NaN`/`Infinity`가 되면 `<v>NaN</v>` 같은 잘못된 XML이 써져
  같은 종류의 손상 경고를 유발할 수 있어 방어적으로 0으로 대체.
- 검증: Node에서 실제로 `buildXlsxWithCharts()`를 호출해 xlsx를 생성한 뒤, 압축을 풀어
  `xl/_rels/workbook.xml.rels`에 styles relationship이 정상 포함됨을 확인하고,
  Python `openpyxl`로 열어 헤더 셀의 볼드 스타일이 정상적으로 읽히는 것도 확인함
  (이 세션 환경에서는 실제 Microsoft Excel 실행이 불가해 그 대안으로 검증).

---

## 2026-7-6 변경 사항 (낙하 물리 핵심 버그 수정 · 착지 후 계속 도는 문제 수정 · 나무 등 재질 강도/파괴 임계점 재검토)

`2026-7-4` 기반. 세 가지를 고쳤습니다: (1) 낙하 시뮬레이션의 고도 적분 부호 버그,
(2) 물체/파편이 바닥에 닿은 뒤에도 계속 회전하는 문제, (3) 나무 등 재질의 충격 파괴 임계점.

### 1. 낙하 시뮬레이션 고도 적분 부호 버그 (핵심 수정)
- `physics.cpp`의 `simulate()` 루프는 `vy`를 **음수 = 하강**으로 다루는 좌표계를 쓰는데,
  고도 갱신이 `altitude -= vy * dt;`로 되어 있어 물체가 하강 가속을 받는 동안에도 오히려
  **고도가 계속 올라가는** 부호 반전 버그가 있었다. (직접 추적해보면 낙하 시작 후 1초 넘게
  고도가 입력값보다 커지는 것을 확인함 — 예: 20m 낙하인데 고도가 20.4m까지 올라감.)
- 이 버그가 물 저항 계산과 겹쳐 더 크게 터졌다: `waterDepth` 계산식이 `input.waterDepth -
  (input.height - altitude)`였는데, 고도가 잘못 올라가는 바람에 낙하 시작 직후부터 이미
  "물에 잠긴" 것으로 오판해 엉뚱한 저항력이 더해졌고, 그 결과 충돌 직전 속도가 한 스텝만에
  두 자릿수로 튀는 수치 폭주가 재현됨(예: 500m 낙하 볼링공의 충돌 속도가 종단속도(64.6 m/s)를
  넘는 등 물리적으로 불가능한 값이 나옴).
- 수정: 고도 갱신을 `altitude += vy * dt;`로 바로잡고, `waterDepth`는 지면 기준 고도만으로
  `std::max(0.0, input.waterDepth - altitude)`로 계산하도록 변경(낙하 시작 시점엔 항상 0,
  물체가 수면 아래로 내려간 만큼만 증가). 수정 후 500m 낙하 볼링공 충돌 속도는 61.7 m/s로
  종단속도(64.6 m/s) 이내에 정상적으로 수렴함을 확인.

### 2. 바닥에 닿은 뒤에도 계속 도는 문제 수정
- 파편 낙하 스텝(`physics.cpp`의 `stepFragments()`)에서 지면 접촉 시 회전(각속도)을 X/Z축만
  감쇠시키고 **수직축(Y, 요잉 회전)은 감쇠하지 않아** 파편이 바닥에 앉은 뒤에도 팽이처럼
  계속 도는 것처럼 보이는 버그가 있었음 → 세 축 모두 감쇠하도록 수정, 완전히 멈출 때
  각속도도 0으로 초기화.
- 낙하 물체 착지 후 바운스 애니메이션(`renderer.js`)에서도 회전을 매 프레임 고정 속도로
  돌리고 있어, 바운스가 거의 잦아들어 실질적으로 멈춘 상태에서도 여전히 빠르게 회전하는
  것처럼 보였음 → 회전 속도를 남은 바운스 속도(에너지)에 비례해 감쇠시키도록 수정.

### 3. 나무 등 재질 강도 / 파괴 임계점 재검토
- 기존에는 충격에 약한 재질(브리틀) 보정이 `fractureMode === "shatter"`(유리)에만 적용되어,
  나무·콘크리트·벽돌 같은 "fracture" 모드 재질은 정적 강도값(`yieldStrength`)을 그대로
  써서 실제보다 훨씬 잘 버티는 것으로 계산되고 있었다. 나무는 결(grain)을 따라 쪼개지는
  특성상 정적 굽힘강도보다 순간 충격에는 훨씬 약하게 반응하는데 이게 반영이 안 되고 있었음.
- `calcImpact()`에 재질별 충격 취약도(brittle factor)를 추가: 나무 3.0배, 콘크리트/벽돌
  1.4배, 유리(shatter)는 기존 8배 유지, 철(deform, 연성 변형)은 변화 없음(그대로 1배).
  같은 정적 강도값이라도 나무가 콘크리트보다 충격에는 더 쉽게 파괴되도록 조정.
- 검증: 500kg 쇠구슬을 30m에서 떨어뜨리면 나무판(10cm)은 "Severe Damage"로 파괴되지만
  같은 조건에서 콘크리트 바닥(40cm)은 견뎌내는 것으로 확인 — 재질별 상대적 강도 차이가
  이제 물리적으로 말이 되는 방향으로 반영됨.

> **C++ 물리 코드(`physics/physics.cpp`)를 수정했습니다. 반드시 `npm run build-addon` 재실행** 후 실행하세요.

---

## 2026-7-4 변경 사항 (충격량/충격력이 음수로 표시되던 버그 수정)

`2026-7-1(5)` 기반. 에너지 대시보드의 **Momentum(운동량)**, **Avg Impact Force(평균 충격력)** 값이
가끔 음수로 표시되던 문제 수정.

### 원인
- `physics.cpp`의 낙하 시뮬레이션 루프는 y축 아래 방향을 **음수**로 다루는 내부 좌표계를 사용
  (`vy`가 낙하 중에는 음수). `calcImpact()`가 이 부호 있는 `impactVelocity`를 그대로
  운동량(`J = mass * impactVelocity`)과 평균 충격력(`F_avg = J / collisionTime`) 계산에 사용해,
  결과값의 부호가 낙하 방향(음수)을 그대로 물려받았음.
- `impactEnergy`는 속도의 제곱이라 부호 문제가 드러나지 않았고, `terminalVelocity`도 `sqrt()`라
  항상 양수였기 때문에 유독 Momentum/Force 두 값만 음수로 보였던 것.
- 실제로 `renderer.js`의 다른 여러 지점(파편 애니메이션, CSV 내보내기 등)은 이미
  `Math.abs(simResult.impactVelocity)`로 방어적으로 절댓값을 취하고 있었고, JS 폴백 구현체인
  `physics-core.js` 역시 `impactVelocity: Math.abs(last.v)`로 절댓값을 반환하고 있어, C++ 쪽만
  부호를 정규화하지 않고 있었던 것으로 확인됨.

### 수정
- `physics/physics.cpp`의 `calcImpact()` 진입 시점에 `impactVelocity = std::abs(impactVelocity);`
  한 줄을 추가해 부호를 한 곳에서 정규화. 이후 계산되는 `impactVelocity`, `impactMomentum(J)`,
  `impactForce(F_avg)`, `impactEnergy`가 모두 (터미널 속도와 동일하게) 항상 양수 크기로 반환됨.
  물리 로직/공식 자체는 변경 없음 — 순수하게 출력 부호만 정규화.

> **C++ 물리 코드(`physics.cpp`)를 수정했습니다. 반드시 `npm run build-addon` 재실행** 후 실행하세요.

---

## 2026-7-1(5) 개선 사항 (렌더 사라짐 버그 · %종단속도 표시 · GLB Cd 자동 추정)

`2026-7-1(4)` 기반. 사용자 보고 버그 2건 수정 + GLB 항력계수 자동 추정 기능 추가.

### 1. 설정 패널을 숨기면 3D 렌더가 전부 사라지던 버그 수정
- **진짜 원인 (Chromium 재현으로 확인)**: 설정 패널(좌)과 리사이저를 `display:none` 으로 숨기면
  CSS 그리드에서 그 항목들이 **배치 흐름에서 빠지고**, 남은 항목들이 자동으로 앞 컬럼으로 밀린다.
  그 결과 3D 캔버스(`#canvas-wrap`)가 `1fr` 컬럼이 아니라 **폭 `0px` 컬럼**에 들어가 폭이 0이 되고,
  아무것도 안 보이게 됨(검은 화면). — 실제로 헤드리스 Chromium 에서 패널을 숨긴 뒤
  `canvasWrap.clientWidth` 가 `0` 이 되는 것을 재현/확인했고, 컬럼 고정 후 `1100px` 로 정상 복구됨.
- **수정**:
  - `.workspace` 의 네 자식(`left-panel`/`panel-resizer`/`#canvas-wrap`/`right-panel`)을
    각각 `grid-column: 1~4` 로 **고정**해, 일부를 `display:none` 으로 숨겨도 캔버스가 항상
    3번 컬럼(`1fr`)에 머물게 함. 시뮬레이터 전용 모드(컬럼 1개)에서는 `#canvas-wrap` 을 컬럼 1로 되돌림.
  - 추가 방어: `resize3()` 에 폭/높이 0 가드(투영행렬 손상 방지), 레이아웃 전환 시 `resize3()` 직접 호출.

### 2. 고고도 낙하 시 %종단속도가 실제 속도와 안 맞던 문제 수정
- **증상**: 매우 높은 곳에서 떨어지면 속도가 급상승해 상단에 표시된 종단속도(해수면 기준)를
  훨씬 넘어서는데도 데이터 표의 `% TERMINAL` 은 85% 등 100% 미만으로 표시됨.
- **원인**: `% TERMINAL` 을 **현재 고도의 국소 종단속도**(공기가 희박한 고고도에서는 훨씬 큼)
  기준으로 계산해 항상 100% 이하로 나오도록 되어 있었음. 이 값이 상단에 표시되는 해수면 기준
  종단속도와 달라 혼란을 줌.
- **수정**: `% TERMINAL` 을 **상단에 표시되는 해수면 기준 종단속도** 대비로 계산하도록 변경.
  고고도에서 실제 속도가 이 종단속도를 넘으면 100%를 초과해 표시된다(막대 그래프만 100%에서 포화).
  데이터 표·CSV·XLSX 내보내기 모두 동일하게 반영.

### 3. GLB/STL 모델의 항력계수(Cd) 자동 추정
- 모델을 불러오면 그 **형상을 분석해 Cd 를 자동으로 추정**해 입력칸에 채운다(이후 수동 수정 가능).
- 실제 Cd 는 CFD 로만 정확히 구할 수 있으므로, 물리적으로 타당한 범위(0.04~2.1)의 **근사값**을
  기하학적 특징으로 계산한다(낙하 방향은 수직으로 가정):
  - **유선형 정도**(fineness = 낙하방향 길이 / √정면단면적): 길쭉할수록 Cd ↓
  - **채움비**(모델 부피 / 바운딩박스 부피, 삼각형 기반 부피 적분): 뭉툭·각질수록 Cd ↑
  - **정면 종횡비**: 납작하거나 길쭉한 판일수록 Cd ↑
- 예: 유선형 막대 ≈ 0.26, 구 ≈ 0.7, 정육면체 ≈ 0.88, 납작한 원반 ≈ 1.4 정도로 추정됨.
- GLB 프리셋을 골라도 형상 기준으로 다시 계산되며, 추정값은 토스트로 안내된다.

> C++ 물리 코드 변경 없음(렌더러 JS만 수정) → `npm run build-addon` 재실행 불필요.
> `npm start` 로 재번들 후 실행하면 됩니다.
