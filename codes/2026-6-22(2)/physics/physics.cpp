...
double PhysicsEngine::calcAirDensity(double alt, double tempOffset, double humidity) {
    ...
    if (alt <= 11000.0) {
        ...
        double rho32 = calcAirDensity(32000.0, tempOffset, humidity);
    ...
    // 습도 보정: Magnus 공식으로 포화증기압 계산
    if (humidity > 0.0 && T > 200.0) {
        double Tc  = T - 273.15;
        double e_s = 611.2 * std::exp(17.67 * Tc / (Tc + 243.04)); // 포화증기압 (Pa)
        double e   = (humidity / 100.0) * e_s;
        // 습윤공기 밀도는 건조공기보다 약간 가볍다
        rho *= (1.0 - 0.378 * e / P);
    }
...
// [F1] 바람(windX, windZ)에 의한 수평 이동 포함, [F8] 기온/습도 반영
ImpactResult PhysicsEngine::simulate(const SimInput& input) {
    ...
    while (altitude > 0.0 && t < 7200.0) {
        ...
        double netFy    = obj.mass * g - sign_vy * drag_y;
        double accel_y  = netFy / obj.mass;

        // 수평 바람 항력: 물체-바람 상대속도에 비례
        double vRelX   = vx - Wx;
        double vRelZ   = vz - Wz;
        double accel_x = -(0.5 * rho * obj.cd * obj.area * vRelX * std::abs(vRelX)) / obj.mass;
