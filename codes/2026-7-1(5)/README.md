...
- **원인**: 레이아웃 전환 순간 `canvas-wrap` 의 폭/높이가 0이 되는 타이밍에 `resize3()` 가
  `camera.aspect = w/h` 를 `0` / `Infinity` / `NaN` 으로 계산 → 투영행렬이 깨져 이후 아무것도
  렌더되지 않았다(검은 화면). 게다가 레이아웃 변경 시 `window` 'resize' 이벤트만 쏘았는데,
  그 리스너는 그래프만 다시 그릴 뿐 `resize3()` 를 호출하지 않아 캔버스가 새 크기로 갱신되지 않았다.
- **수정**:
  - `resize3()` 에 폭/높이가 1px 미만이면 갱신을 건너뛰는 가드 추가(투영행렬 손상 방지).
  - 패널 표시/숨김·시뮬레이터 전용 모드 전환 시 `resize3()` 를 **직접** 호출(레이아웃 확정 후
    `requestAnimationFrame` 2회 지연)하고 그래프도 다시 그리도록 변경.
