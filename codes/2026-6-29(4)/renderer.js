(async () => {
  ...
  // 메뉴/결과창 이벤트 연결
  if(window.appBridge){
    window.appBridge.onMenuUpload(p=>{
      // 숨겨진 파일 입력을 직접 클릭. file-shape-glb=.glb, file-stl=.stl/.glb
      const inp = (p&&p.kind==='glb') ? ($('file-shape-glb')||$('file-stl')) : $('file-stl');
      if(inp) inp.click();
    });
  ...
  function setupMenubar(){
    ...
    const mvG=$('mv-graph'), mvT=$('mv-traj'), mvS=$('mv-settings'), mvSO=$('mv-simonly');
    ...
    if(mvSO) mvSO.addEventListener('change',()=>applyViewSetting('simOnly',mvSO.checked));
    syncMenubar();
  }
  function syncMenubar(){
    ...
    if(mvSO) mvSO.checked=view.simOnly;
